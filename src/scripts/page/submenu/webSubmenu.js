...
function toChat() {
    var submenu = $('.everyTitle')
    var str = "";
    for (var m = 0; m < submenu.length; m++) {
        for (var i = 0; i < supplierData.length; i++) {
            if (supplierData[i].supplierName === submenu[m].innerText) {
                for (var j = 0; j < supplierData[i].seatGroups.length; j++) {
                    str += "<li>" + supplierData[i].seatGroups[j].groupName + "</li>"
                    document.getElementById(submenu[m].innerText).innerHTML = str;
                }
                str = "";
            }
        }
